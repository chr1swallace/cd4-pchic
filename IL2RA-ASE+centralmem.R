...
## setkeyv(x, c("time","stim","Expt","snp"))
## ux <- unique(x,by=c("time","stim","Expt","snp"))

## p <- ggplot(x, aes(x=time,y=exp(logprop),col=stim)) +
## geom_hline(aes(yintercept=exp(mean.logprop)),
##            data=ux[time<0,],col=cols[1],linetype="dashed") +
## geom_point() + facet_grid(Expt~ snp)  + #geom_smooth() +
## geom_path(aes(x=time,y=exp(mean.logprop)),col=cols[3],data=x[stim %in% c("time0","non-activated"),]) +
## geom_path(aes(x=time,y=exp(mean.logprop)),col=cols[4],data=x[stim %in% c("time0","activated"),]) +
## geom_segment(aes(x=time-w,xend=time+w,y=exp(mean.logprop),yend=exp(mean.logprop)),size=3) +
