...
plotter <- function(nm,plus=NULL) {
    ...
    trt.r <- with(tracks, list(gwast, genes,
                               rnan, rnaa, ac27n, ac27a, 
                               reg))
    ...
    sz.r <- c(rep(0.4,2),rep(0.5,length(trt.r)-3),0.3)
    trt <- c(trt.l, trt.r)
    sz <- c(0.3,sz.l,sz.r)
...
## check AHR/RA OKADA
ra<-fread("/scratch/ob219/transfer/RA_OKADA_IMB.pmi")
sle<-fread("/scratch/ob219/transfer/SLE.pmi")
head(fm)
ra <- ra[ppi>0.01,]
sle <- sle[ppi>0.01,]
setnames(ra,c("start","rsid","ppi"),c("position","snp","mppi"))
setnames(sle,c("start","rsid","ppi"),c("position","snp","mppi"))
ra[,ppsum:=ifelse(mppi>0.01,1,0)]
sle[,ppsum:=ifelse(mppi>0.01,1,0)]
ra[,tag:="gwas"]
sle[,tag:="gwas"]
ra[,trait:="RA-OKADA"]
sle[,trait:="SLE"]

fm <- rbind(fm[,.(chr,position,snp,trait,mppi,ppsum,tag)],
            ra[,.(chr,position,snp,trait,mppi,ppsum,tag)],
            sle[,.(chr,position,snp,trait,mppi,ppsum,tag)])
...
trove2=list(disease=c("CEL"## ,"MS"
            ...
            to=193120000),
               ...
               irf8=list(disease=c(## "CRO","MS","PBC",
                         ...
                         from=85790000,
                         to=86100000),
               ahr=list( disease=c("RA","RA-OKADA"),
                         chromosome="7",
                         from=16900000,
                        to=17416562),
...
for(nm in names(doplot)) {
    ...
    f <-file.path(CD4CHIC.OUT,paste0("paper3/",nm,"-square-allgwas.pdf"));
    ## if(file.exists(f))
    ##     next
    message(f)
    if(nm=="il10") {
