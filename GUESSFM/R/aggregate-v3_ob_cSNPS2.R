...
#setwd("/home/oliver/GIT_REPOS/cd4chic/GUESSFM/R/")
...
#setwd("/home/chrisw/Projects/cd4chic/GUESSFM")
#args <- list(d=file.path("/stats/chrisw/FM-GUESS","10p-30686933-30807686"))
...
print(names(GUESS.ichip.grl))
agg.gr<-GUESS.ichip.grl[['overall']]
...
#dirs <- list.files("/stats/chrisw/FM-GUESS",full=TRUE)
...
# just examine
#stop();
#dd<-'/scratch/cew54/FM-GUESS/19p-10396336-10628468'



for(dd in dirs) {    
    chr <- sub("[pq].*","",basename(dd))
    #f.geno <- paste0("/stats/chrisw/FM-impute_qc/imputed-",basename(dd),".RData")
    f.geno <- paste0("/scratch/cew54/FM-impute_qc/imputed-",basename(dd),".RData")
    f.sm <- file.path(dd, "snpmod-fixed2.RData")
    #f.sm <- file.path(dd, "snpmod.RData")    
    if(!file.exists(f.sm))
        next
    message("\n",dd)
...
## summary of GUESSFM
    ...
    results <- mclapply(all.ensg.ld, function(i) {
        ...
        MPPI.overlap(SM2,
                     in.gr = csnps[ csnps$ensg==g & csnps$ld.id==l ],
                     out.gr = csnps[csnps$ensg!=g]) 
    }, mc.cores=24)
...
#library(ggplot2)
#ggplot(agg2,aes(x=T1D,y=JIA)) + geom_point()

#library(pheatmap)
#cor(agg2[,DISEASES,with=FALSE]) %>% pheatmap()
